...
- The hook and the scripts build the wheel that the `[tool.h2py]` table of its `pyproject.toml` describes (module, foreign library, Cabal package, smoke test, test suite and its requirements), resolved by `scripts/wheel-config.py` with defaults from `project.name`, from the packaging directory `H2PY_PACKAGE_DIR` names (default `h2py-examples/python`); `configure-python.sh` writes the include directory for `h2py` and the packages in `H2PY_CABAL_PACKAGES` (default `h2py-examples`).
  `h2py-examples` is one such wheel, and the "Shipping wheels" section of `docs/tutorial.md` takes a project of one's own through the same scripts, so a change to their interface, or to what they expect of a project, updates that section too.
